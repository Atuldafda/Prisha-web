  ...
  return (
    <section id="contact" className="py-24 px-6 bg-brand-navy relative overflow-hidden leading-relaxed font-medium">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10 leading-relaxed">
        ...
        <p className="text-brand-sky text-xl mb-12 font-semibold opacity-90 max-w-2xl mx-auto leading-relaxed font-medium">
          Let's discuss your project and how Prisha Web can help you achieve your digital goals.
        </p>
        <form className="bg-white p-8 md:p-12 rounded-brand-lg shadow-brand-soft text-left leading-relaxed">
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            <div>
              <label className="block text-sm font-bold text-brand-navy mb-2 uppercase tracking-wide">Name</label>
              <input type="text" className="w-full px-4 py-4 rounded-brand-sm border border-slate-200 focus:ring-2 focus:ring-brand-sky focus:border-transparent outline-none transition-all font-medium text-brand-slate-dark" placeholder="John Doe" />
            </div>
            <div>
              <label className="block text-sm font-bold text-brand-navy mb-2 uppercase tracking-wide font-medium">Email</label>
              <input type="email" className="w-full px-4 py-4 rounded-brand-sm border border-slate-200 focus:ring-2 focus:ring-brand-sky focus:border-transparent outline-none transition-all font-medium text-brand-slate-dark" placeholder="john@example.com" />
            </div>
          </div>
          ...
          <div className="mb-10 leading-relaxed font-medium">
            ...
            <textarea className="w-full px-4 py-4 rounded-brand-sm border border-slate-200 focus:ring-2 focus:ring-brand-sky focus:border-transparent outline-none transition-all h-40 font-medium text-brand-slate-dark resize-none" placeholder="Tell us about your project..."></textarea>
          </div>
          <button className="w-full bg-brand-navy text-white py-5 rounded-brand-sm font-extrabold text-lg hover:bg-brand-slate-dark transition-all shadow-lg">
            Send Inquiry
          </button>
        </form>
      </div>
